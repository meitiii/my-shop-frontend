...
export default function SearchPage() {
  // گرفتن slug از آدرس (مثل /category/mobile)
  ...
  // 👈 جادوی تبدیل Slug به ID
  useEffect(() => {
    if (categories.length > 0) {
      if (slug) {
        // یک تابع بازگشتی برای پیدا کردن دسته‌بندی تو همه سطوح (پدر، فرزند، نوه)
        ...
        if (matchedCat) {
          ...
          setSelectedCategory(null); // اگر slug نامعتبر بود
        }
      } else {
        // اگر تو صفحه /search بودیم و slug نداشتیم
  ...
  // تغییر مسیر URL هنگام انتخاب دسته‌بندی از سایدبار
  const handleCategoryChange = (cat: any | null) => {
    if (cat) {
      // میره به آدرس جدید دسته‌بندی، ولی بقیه پارامترهای سرچ رو نگه می‌داره
      navigate({
        pathname: `/category/${cat.slug}`,
        search: location.search
      });
    } else {
      // اگر روی All Products زد، برمی‌گرده به صفحه جستجوی اصلی
      navigate({
        pathname: `/search`,
        search: location.search
      });
  ...
  const FiltersSidebar = () => (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="font-bold text-gray-900 text-lg flex items-center gap-2"><SlidersHorizontal size={18}/> Filters</h2>
        ...
        <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-2">
          {brands?.map((brand: any) => (
            <label key={brand.id} className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={selectedBrands.includes(brand.id)} onChange={() => toggleBrand(brand.id)} className="w-4 h-4 rounded text-blue-600 border-gray-300" />
              <span className={`text-sm ${selectedBrands.includes(brand.id) ? 'font-bold text-gray-900' : 'text-gray-600'}`}>{brand.name}</span>
            </label>
          ))}
        </div>
      </div>
    ...
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="w-full md:w-[400px] relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
            />
            {searchInput && <button onClick={() => setSearchInput('')} className="absolute inset-y-0 right-0 pr-3"><X className="h-4 w-4 text-gray-400" /></button>}
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="hidden md:block w-64 flex-shrink-0"><FiltersSidebar /></div>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-6 bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 overflow-x-auto hide-scrollbar">
              <span className="text-gray-500 font-bold text-sm flex items-center gap-1.5 ml-2">Sort by:</span>
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-semibold whitespace-nowrap transition-colors ${ordering === opt.value ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                    {opt.label}
                  </button>
                ))}
            ...
            {productsLoading ? (
              ...
              <div className="text-center py-12 bg-white rounded-2xl border border-red-100 text-red-500 shadow-sm">Failed to load products.</div>
            ) : !products || products.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                <h3 className="text-lg font-bold text-gray-900 mb-2">No products found</h3>
                <button onClick={clearFilters} className="px-5 py-2 bg-blue-50 text-blue-700 font-bold rounded-lg hover:bg-blue-100">Clear All Filters</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map((product: Product) => {
                  ...
                  return (
                    <Link to={`/product/${product.id}`} key={product.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md border border-gray-100 transition-all overflow-hidden flex flex-col justify-between group">
                      <div className="relative p-4">
                        {discountPercent > 0 && <span className="absolute top-4 right-4 bg-red-500 text-white text-xs font-black px-2.5 py-1 rounded-md z-10 shadow-sm">{discountPercent}% OFF</span>}
                        <div className="h-48 rounded-xl bg-gray-50 flex items-center justify-center overflow-hidden mb-4">
                          {coverImage ? (
                            <img src={coverImage.image} alt={coverImage.alt_text || product.name} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500 p-2" />
                          ) : (
